...
import 'package:bump/core/services/database_service.dart';
import 'package:bump/core/services/shake_detector.dart';
import 'package:bump/features/bump/widgets/bump_match_dialog.dart';
import 'package:bump/features/home/home_screen.dart';
import 'package:bump/features/settings/settings_provider.dart'; // [필수] 설정 확인용 임포트
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
...
class _BumpScreenState extends ConsumerState<BumpScreen> {
  ...
  Future<void> _startScanAudio() async {
    try {
      await _audioPlayer.setVolume(0.5);
      await _audioPlayer.setReleaseMode(ReleaseMode.loop);
      await _audioPlayer.play(AssetSource('sounds/radar_scan.mp3'));
    } catch (e) {
      debugPrint("오디오 오류: $e");
    }
  }
  ...
  // lib/features/bump/bump_screen.dart 내부
  ...
  Future<void> _startBumpProcess() async {
    ...
    final dbService = ref.read(databaseServiceProvider);
    final modeIndex = ref.read(modeProvider); // 현재 선택된 모드 (0, 1, 2)
    final modeKey = ['business', 'social', 'private'][modeIndex];

    try {
      final userData = await dbService.getUserData(user.uid);
      
      // [기존 코드] 단순히 프로필만 가져옴 (여기에는 modeIndex가 저장되어 있지 않음)
      // final myProfile = (userData?['profiles'] as Map?)?[modeKey] ?? {'name': 'Unknown'};

      // [수정된 코드] 프로필 데이터를 복사한 뒤, 'modeIndex'를 명시적으로 추가함!
      final originalProfile = (userData?['profiles'] as Map?)?[modeKey] ?? {'name': 'Unknown'};
      final Map<String, dynamic> dataToSend = Map<String, dynamic>.from(originalProfile);
      
      // ★ 핵심 해결책: "이 명함은 Social 모드(1)입니다"라고 도장을 찍어서 보냄
      dataToSend['modeIndex'] = modeIndex; 

      // 수정된 dataToSend를 전송
      String reqId = await dbService.createBumpRequest(user.uid, dataToSend);
